module.exports = (io) => {
    ...
    io.on('connection', (socket) => {
        ...
        // --- Game Logic ---
        const games = new Map(); // gameId -> { type, players: [id1, id2], board, turn, status, winner }

        socket.on('game_invite', ({ targetId, gameType }) => {
        ...
        socket.on('game_accept', ({ inviterId, gameType }) => {
            const gameId = `${socket.id}-${inviterId}-${Date.now()}`;
            const initialBoard = Array(9).fill(null); // Tic Tac Toe specific for now

            const gameState = {
                id: gameId,
                type: gameType,
                players: [inviterId, socket.id], // Player X (inviter), Player O (acceptor)
                board: initialBoard,
                turn: inviterId,
                status: 'playing',
                winner: null
            ...
            // Notify both players
            [inviterId, socket.id].forEach(playerId => {
        ...
        socket.on('game_reject', ({ inviterId }) => {
            io.to(inviterId).emit('game_rejected', { rejectorId: socket.id });
        });

        socket.on('game_move', ({ gameId, index }) => {
            const game = games.get(gameId);
            if (!game || game.status !== 'playing') return;
            if (game.turn !== socket.id) return; // Not your turn
            if (game.board[index] !== null) return; // Cell occupied
            ...
            game.players.forEach(playerId => {
                io.to(playerId).emit('game_update', {
                    board: game.board,
                    turn: game.turn,
                    status: game.status,
                    winner: game.winner
            ...
            if (game.status !== 'playing') {
                // Cleanup after a delay or keep it for "Play Again"? 
                // For now, keep it in memory until players leave or restart
            }
        });
            ...
            if (game) {
                game.players.forEach(playerId => {
                    if (playerId !== socket.id) {
                        io.to(playerId).emit('game_ended', { reason: 'opponent_left' });
                    }
                });
                games.delete(gameId);
            }
