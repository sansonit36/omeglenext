...
const GroupVideoChat = ({ socket, roomId, onLeave }) => {
    ...
    return (
        <div className="h-screen flex flex-col bg-[#0f0f13] text-white">
            {/* Header */}
            <header className="h-16 border-b border-white/10 flex items-center justify-between px-6 bg-[#1a1a23]">
                ...
                <div className="flex items-center gap-4">
                    {isAdmin && <span className="px-3 py-1 bg-indigo-500/20 text-indigo-400 text-xs font-bold rounded-full border border-indigo-500/30">ADMIN</span>}
            ...
            <div className="flex-1 p-4 overflow-y-auto">
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-fr h-full">
                    ...
                    <div className="relative bg-black rounded-2xl overflow-hidden border border-white/10 aspect-video">
                        ...
                        />
                        <div className="absolute bottom-3 left-3 bg-black/60 px-2 py-1 rounded text-xs font-medium">You</div>
                        <div className="absolute top-3 right-3 flex gap-2">
                            <button onClick={() => setMicOn(!micOn)} className={`p-2 rounded-full ${micOn ? 'bg-white/10' : 'bg-red-500 text-white'}`}>
                                {micOn ? <Mic size={14} /> : <MicOff size={14} />}
                            </button>
                            <button onClick={() => setCameraOn(!cameraOn)} className={`p-2 rounded-full ${cameraOn ? 'bg-white/10' : 'bg-red-500 text-white'}`}>
                    ...
                    {/* Remote Peers */}
                    {peers.map(peer => (
                        <div key={peer.userId} className="relative bg-black rounded-2xl overflow-hidden border border-white/10 aspect-video">
                            <video
                                autoPlay
                                playsInline
                                ref={el => {
                            ...
                            {isAdmin && (
                                <div className="absolute top-3 right-3 flex gap-2">
                                    <button
                                        ...
                                        className={`p-2 rounded-full ${peer.isMuted ? 'bg-red-500 text-white' : 'bg-white/10 hover:bg-white/20'}`}
